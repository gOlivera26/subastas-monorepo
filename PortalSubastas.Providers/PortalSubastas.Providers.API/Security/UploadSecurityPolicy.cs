...
namespace PortalSubastas.Providers.API.Security;

public static class UploadSecurityPolicy
{
    public const long MaxDocumentBytes = 20 * 1024 * 1024;
    public const long MaxCsvBytes = 5 * 1024 * 1024;
    ...
    public static string? ValidateCsv(IFormFile? file, IConfiguration? configuration = null)
    {
        if (file is null || file.Length == 0)
        {
            return "Debe adjuntar un archivo CSV.";
        }

        var maxBytes = configuration.GetUploadValue("Csv:MaxBytes", MaxCsvBytes);
        if (file.Length > maxBytes)
        {
            return $"El archivo CSV no puede superar los {BytesToMb(maxBytes)} MB.";
        }
        ...
        var allowedContentTypes = configuration.GetUploadValues("Csv:AllowedContentTypes", AllowedCsvContentTypes);
        if (!allowedContentTypes.Contains(file.ContentType, StringComparer.OrdinalIgnoreCase))
        {
            return "El tipo de contenido del CSV no es válido.";
        }
